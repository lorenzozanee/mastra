...
export function createHallucinationScorer({
  ...
  return createScorer<ScorerRunInputForLLMJudge, ScorerRunOutputForLLMJudge>({
    ...
    .generateScore(({ results }) => {
      const totalStatements = results.preprocessStepResult?.claims?.length ?? 0;
      const contradictedStatements = results.analyzeStepResult.verdicts.filter(v => v.verdict.toLowerCase().trim() === 'yes').length;
