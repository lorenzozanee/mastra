...
describe('KnowledgeRoutes', () => {
  it('keeps graph, activity, and detail reads on the requested key without fallback', async () => {
    ...
    for (const endpoint of ['importers', 'importers/calendar/runs', 'importers/calendar/runs/run-1']) {
      resolve.mockClear();
      const response = await h.app.request(
        `/web/factory/projects/${h.projectId}/knowledge/${endpoint}?knowledgeKey=second`,
      );
      expect(response.status).toBe(endpoint === 'importers' ? 200 : 404);
      expect(resolve).toHaveBeenCalledWith('second');
    }
